...
var coche, avion,audi, dodge, nissan, ferrari, touareg, truck, dragon, boat;
var avionsalto, avionvolver;
var cochey, cochex, cochez;
var focal, ambiental, direccional;
////////////////////////////
...
function init() {
	...
	direccional.position.set( 0,60,0 );
	//direccional.castShadow = true;
	scene.add( direccional );

	/*var puntual = new THREE.PointLight( 0xffaa00,1, 100 );
	puntual.position.set( 2,60,-4 );
	puntual.castShadow = true;
	scene.add( puntual );
	//Set up shadow properties for the light
	puntual.shadow.mapSize.width = 512; // default
	puntual.shadow.mapSize.height = 512; // default
	puntual.shadow.camera.near = 0.5; // default
	puntual.shadow.camera.far = 500; // default*/

	focal = new THREE.SpotLight( 'black', 2 );
	focal.position.set( 0, 80, 0 );
	focal.target.position.set( 120,120,120);
...
//VIDEO
    ...
    video.src = "videos/zaraLarsson-NeverForgetYou.mp4";
    video.muted = true;
    //video.volume = 0.25;
    video.load();
    video.play();
	...
	loader2.load( 'models/audi/audi9.json', 
	         function (objeto){
	         	objeto.name = 'audi';
	         	audi = objeto;
				...
				console.log("rotx"+objeto.rotation.x);
				console.log("roty"+objeto.rotation.y);
				console.log("rotz"+objeto.rotation.z);
...
*/
	...
	loader5.load( 'models/camion/mining-dump-truck9.json', 
	         function (objeto){
                ...
                cochex=objeto.rotation.x;
                cochey=objeto.rotation.y;
                cochez=objeto.rotation.z;
				objeto.receiveShadow=true;
				truck=objeto;
		        objeto.castShadow=true;
...
function setupGUI()
{
	...
	// Controles
	effectControls = {
		mensaje: "Proyecto AGM",
		roty: 0,
		rotx: 0,
		rotz: 0,
		luz: 0,
		colorfoc: "rgb(255,255,255)"
	...
	folder.add( effectControls, "roty",-10, 10, 0.01 ).name("Rotar");
	folder.add( effectControls, "rotx", -10, 10, 0.01 ).name("Arriba/Abajo");
	folder.add( effectControls, "rotz", -10, 10, 0.01 ).name("Dcha/Izq");
	folder.add( effectControls, "luz", 0, 5, 0.1 ).name("Luminosidad");
	folder.addColor(effectControls, "colorfoc").name("Color Luz");
...
function update()
{
	...
	if(coche!=null){
		coche.rotation.y = effectControls.roty;
		coche.rotation.x = effectControls.rotx;
		coche.rotation.z = effectControls.rotz;

	}

	focal.intensity = effectControls.luz;
	direccional.intensity = effectControls.luz;
	//convert rgb string a hexadecimal
	var a;
	a='"'+effectControls.colorfoc+'"';
	var l1=a.split("(")[1].split(")")[0];
	l1=l1.split(",");
	...
	focal.color.setHex(l2);
	ambiental.color.setHex( l2);
	direccional.color.setHex( l2);
	video.volume=0.25;
	//video.volumen=effectControls.volumen;
	/*if(effectControls.videoo==0){
		video.muted = false;
		video.volumen=effectControls.volumen;
    	video.load();
    	video.play();
	}
	if(effectControls.videoo==1){
		video.muted = true;
    	video.load();
    	video.play();
	}*/

	//focal.color = effectControls.colorfoc;
	//ambiental.color = effectControls.coloramb;
	//direccional.color = effectControls.colordir;
    ...
    if( interseccion.length > 0){
    	...
    	for(var i=0;i<interseccion.length;i++){
        	...
        	if(interseccion[0].object.name == 'audi'){
        		coche=audi;
	        	effectControls.roty=audi.rotation.y;
	        	effectControls.rotx=audi.rotation.x;
	        	effectControls.rotz=audi.rotation.z;
				audisalto.chain(audivolver);
				audisalto.start();
        	}
        	if(interseccion[0].object.name == 'dodge'){
        		coche=dodge;
	        	effectControls.roty=dodge.rotation.y;
	        	effectControls.rotx=dodge.rotation.x;
	        	effectControls.rotz=dodge.rotation.z;
        		dodgesalto.chain(dodgevolver);
        		dodgesalto.start();
        	}
        	if(interseccion[0].object.name == 'nissan'){
        		coche=nissan;
	        	effectControls.roty=nissan.rotation.y;
	        	effectControls.rotx=nissan.rotation.x;
	        	effectControls.rotz=nissan.rotation.z;
        		nissansalto.chain(nissanvolver);
        		nissansalto.start();
        	}
        	if(interseccion[0].object.name == 'ferrari'){
        		coche=ferrari;
	        	effectControls.roty=ferrari.rotation.y;
	        	effectControls.rotx=ferrari.rotation.x;
	        	effectControls.rotz=ferrari.rotation.z;
        		ferrarisalto.chain(ferrarivolver);
        		ferrarisalto.start();
        	}
        	if(interseccion[0].object.name == 'touareg'){
        		coche=touareg;
	        	effectControls.roty=touareg.rotation.y;
	        	effectControls.rotx=touareg.rotation.x;
	        	effectControls.rotz=touareg.rotation.z;
        		touaregsalto.chain(touaregvolver);
        		touaregsalto.start();
        	}
        	if(interseccion[0].object.name == 'truck'){
        		coche=truck;
	        	effectControls.roty=truck.rotation.y;
	        	effectControls.rotx=truck.rotation.x;
	        	effectControls.rotz=truck.rotation.z;
        	...
        	if(interseccion[0].object.name == 'dragon'){
        		coche=dragon;
	        	effectControls.roty=dragon.rotation.y;
	        	effectControls.rotx=dragon.rotation.x;
	        	effectControls.rotz=dragon.rotation.z;
				dragonsalto.chain(dragonvolver);
        		dragonsalto.start();
        	}
        	if(interseccion[0].object.name == 'boat'){
        		coche=boat;
	        	effectControls.roty=boat.rotation.y;
	        	effectControls.rotx=boat.rotation.x;
	        	effectControls.rotz=boat.rotation.z;
				boatsalto.chain(boatvolver);
        		boatsalto.start();
        	}
        	if(interseccion[0].object.name == 'avion'){
        		coche=avion;
	        	effectControls.roty=avion.rotation.y;
	        	effectControls.rotx=avion.rotation.x;
	        	effectControls.rotz=avion.rotation.z;
